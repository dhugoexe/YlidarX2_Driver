...
int init_lidar(void)
{
    lidar_motorSpeed(LIDAR_MOTOR_STOP);
    status = HAL_UART_Receive_DMA(&huart3, rxBuffer, RX_HEADER_SIZE);
	while(!LidarUartFlag){}
    lidar_motorSpeed(LIDAR_MOTOR_NORMAL_SPEED);
...
lidar_point_t lidar_scan_loop(void)
{
    ...
    if(rxBuffer[0] == HEADER_BYTE1 && rxBuffer[1] == HEADER_BYTE2)
    {
        ...
        lidar_DataProcessing(sampleData, sample);
    }
...
//TODO: Ajuster en fonction du TIM utilisé
void lidar_motorSpeed(uint8_t dutyCycle)
{
    if (dutyCycle >= currentDutyCycle)
	{
		while(currentDutyCycle != dutyCycle)
		{
			htim16.Instance->CCR1=TIMER_TIM_BASE*currentDutyCycle/100;
			currentDutyCycle++;
			HAL_Delay(100);

		}
	}
	else if (dutyCycle <= currentSpeed)
	{
		while(currentSpeed != dutyCycle)
		{
			htim16.Instance->CCR1=TIMER_TIM_BASE*currentDutyCycle/100;
			currentDutyCycle--;
			HAL_Delay(100);
